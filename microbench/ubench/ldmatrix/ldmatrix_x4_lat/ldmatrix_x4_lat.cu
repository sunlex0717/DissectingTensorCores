...
__global__ void shared_lat(uint32_t *startClk, uint32_t *stopClk,
  ...
  //if (uid == 0) {
    ...
    unsigned addr5 = static_cast<unsigned>(__cvta_generic_to_shared(&s[(threadIdx.x + 128) * 4]));
    unsigned addr5_1 = 0;
    unsigned addr5_2 = 0;
    unsigned addr5_3 = 0;

    unsigned addr6 = static_cast<unsigned>(__cvta_generic_to_shared(&s[(threadIdx.x + 160) * 4]));
    unsigned addr6_1 = 0;
    unsigned addr6_2 = 0;
    unsigned addr6_3 = 0;

    unsigned addr7 = static_cast<unsigned>(__cvta_generic_to_shared(&s[(threadIdx.x + 192) * 4]));
    unsigned addr7_1 = 0;
    unsigned addr7_2 = 0;
    unsigned addr7_3 = 0;

    unsigned addr8 = static_cast<unsigned>(__cvta_generic_to_shared(&s[(threadIdx.x + 224) * 4]));
    unsigned addr8_1 = 0;
    unsigned addr8_2 = 0;
    unsigned addr8_3 = 0;
    ...
    for (uint32_t i = 0; i < ITERS; ++i) {
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr), "=r"(addr_1),"=r"(addr_2),"=r"(addr_3) : "r"(addr)); 
        #if ILPconfig >= 2
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr2), "=r"(addr2_1),"=r"(addr2_2),"=r"(addr2_3) : "r"(addr2)); 
        #endif 
        #if ILPconfig >= 3
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr3), "=r"(addr3_1),"=r"(addr3_2),"=r"(addr3_3) : "r"(addr3));
        #endif 
        #if ILPconfig >= 4
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr4), "=r"(addr4_1),"=r"(addr4_2),"=r"(addr4_3) : "r"(addr4)); 
        #endif 

        #if ILPconfig >= 5
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr5), "=r"(addr5_1),"=r"(addr5_2),"=r"(addr5_3) : "r"(addr5)); 
        #endif 

        #if ILPconfig >= 6
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr6), "=r"(addr6_1),"=r"(addr6_2),"=r"(addr6_3) : "r"(addr6)); 
        #endif 

        #if ILPconfig >= 7
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr7), "=r"(addr7_1),"=r"(addr7_2),"=r"(addr7_3) : "r"(addr7)); 
        #endif 

        #if ILPconfig >= 8
        asm volatile ("ldmatrix.sync.aligned.x4.m8n8.shared.b16 {%0, %1, %2, %3}, [%4];" : "=r"(addr8), "=r"(addr8_1),"=r"(addr8_2),"=r"(addr8_3) : "r"(addr8)); 
        #endif 
        __syncwarp();
    }
    ...
    startClk[uid] = start;
    stopClk[uid] = stop;
    dsink[uid] = addr + addr_1 + addr_2 + addr_3;
    dsink[uid] += addr2 + addr2_1 + addr2_2 + addr2_3;

    dsink[uid] += addr3 + addr3_1 + addr3_2 + addr3_3;
    dsink[uid] += addr4 + addr4_1 + addr4_2 + addr4_3;
    dsink[uid] += addr5 + addr5_1 + addr5_2 + addr5_3;
    dsink[uid] += addr6 + addr6_1 + addr6_2 + addr6_3;
    dsink[uid] += addr7 + addr7_1 + addr7_2 + addr7_3;
    dsink[uid] += addr8 + addr8_1 + addr8_2 + addr8_3;
...
void test_with_different_thread(int THREADS_NUM, int ILP){
  ...
  long num_bytes =  (TOTAL_THREADS/32) * 8 * 8 * 2 * 4 * ILP;

  std::cout << THREADS_NUM/32 <<" warps ldmatrix.x4 latency " << lat <<" ( " <<(unsigned)(lat) << " ) " << std::endl;
...
int main() {
  std::vector<int> warps = {1,2,3,4,5,6,7,8,12,16,20};
  //std::vector<int> warps = {4,8,12,16};
  intilizeDeviceProp(0);
  std::cout<<"***********************************"<<std::endl;
  std::cout << "ldmatrix.x4 microbenchmark with ILP = " << ILPconfig << std::endl;
  // std::cout << "ldmatrix.x4 microbenchmark " <<std::endl;
  for(auto &e:warps){
