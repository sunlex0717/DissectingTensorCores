...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    //           (float)total_time;
    fma_bw = ((float)(ITERS * 16 * 8 * 64 * ILPconfig * //0 *
    ...
    std::cout << "mma.sync.aligned.m16n8k64.row.col.s32.s4.s4.s32 latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k64.row.col.s32.s4.s4.s32 microbenchmark with ILP = " << ILPconfig << std::endl;
