...
static_assert(ILPconfig <=6, "ILP>8 is not supported\n");
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 microbenchmark with ILP = " << ILPconfig << std::endl;
