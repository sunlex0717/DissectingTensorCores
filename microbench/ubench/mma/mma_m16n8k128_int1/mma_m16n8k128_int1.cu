...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    //           (float)total_time;
    fma_bw = ((float)(ITERS * 16 * 8 * 128 * ILPconfig * //0 *
    ...
    std::cout << "mma.sync.aligned.m16n8k128.row.col.s32.b1.b1.s32.and.popc latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k128.row.col.s32.b1.b1.s32.and.popc microbenchmark with ILP = " << ILPconfig << std::endl;
