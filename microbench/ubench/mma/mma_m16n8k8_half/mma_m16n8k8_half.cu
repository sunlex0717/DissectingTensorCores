...
__global__ void tensr1688_flops(uint64_t *startClk, uint64_t *stopClk, half *a, half *b, float *res,
  ...
  half frag_B[2*ILPconfig];  // one .f16x2 registers, 4 half  elements
...
float tensor1688_max_flops(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    unsigned total_A_SIZE =
        16*8 * (TOTAL_THREADS / WARP_SIZE); // asume one 16x8 matrix per warp
    unsigned total_B_SIZE =
        8*8 * (TOTAL_THREADS / WARP_SIZE); // asume one 8*8 matrix per warp
    ...
    //           (float)total_time;
    fma_bw = ((float)(ITERS * 16 * 8 * 8 * ILPconfig * 
    ...
    std::cout << "mma.sync.aligned.m16n8k8.row.col.f16.f16.f16.f16  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k8.row.col.f16.f16.f16.f16 microbenchmark with ILP = " << ILPconfig << std::endl;
    for(int i = 1; i <= 32; i = i*2){
        std::cout << "Number of warps = "<< i <<std::endl;
        tensor1688_max_flops<half, float>(32*i);
        std::cout << std::endl;
    }
