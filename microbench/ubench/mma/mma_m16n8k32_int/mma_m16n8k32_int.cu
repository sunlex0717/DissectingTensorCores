...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sync.aligned.m16n8k32.row.col.s32.s8.s8.s32 latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k32.row.col.s32.s8.s8.s32 microbenchmark with ILP = " << ILPconfig << std::endl;
