...
const static int mma_m = 16;
const static int mma_n = 8;
const static int mma_k = 32;
...
__global__ void mma_ubench(uint64_t *startClk, uint64_t *stopClk, char *a, char *b, int *res,
  ...
  a = a + warpid * mma_m*mma_k; // m*k = 8*16
  b = b + warpid * 8*16; // n*k = 8*16
  res = res + warpid * 16*8;// m*n = 8*8

   /** step 1: create register for each thread **/
  char frag_A[8*ILPconfig]; // two b32 registers, each contains eight s4/u4
  char frag_B[4*ILPconfig];  // one b32 register, each contains eight s4/u4
  int frag_D[4*ILPconfig]; //4 s32 register
...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sync.aligned.m16n8k32.row.col.s32.s4.s4.s32 latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k32.row.col.s32.s4.s4.s32 microbenchmark with ILP = " << ILPconfig << std::endl;
