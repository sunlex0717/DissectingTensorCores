...
__global__ void mma_ubench(uint64_t *startClk, uint64_t *stopClk, float *a, float *b, float *res,
  ...
  res = res + warpid * 16*8;// m*n = 16*16
   ...
   __nv_bfloat16 frag_A[8*ILPconfig]; // two .f16x2 registers, 8 half elements, 
   __nv_bfloat16 frag_B[4*ILPconfig];  // one .f16x2 registers, 4 half  elements
...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    ...
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32 microbenchmark with ILP = " << ILPconfig << std::endl;
    for(auto& e:warps){
        std::cout << "Number of warps = "<< e <<std::endl;
        run<float, float>(32*e);
        std::cout << std::endl;
    }
