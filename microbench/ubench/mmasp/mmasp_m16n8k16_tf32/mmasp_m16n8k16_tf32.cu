...
__global__ void mmasp_16816(uint64_t *startClk, uint64_t *stopClk, float *a, float *b, uint32_t* meteE, float *res,
  ...
  res = res + warpid * 16*8;// m*n = 16*16
   ...
   float frag_A[4*ILPconfig]; // two b32 registrs, 4 half non-zero elements, 16 dense 
   float frag_B[4*ILPconfig]; // two f16x2 registers, 8 half dense elements
...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f32.tf32.tf32.f32  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f32.tf32.tf32.f32  microbenchmark with ILP = " << ILPconfig << std::endl;
