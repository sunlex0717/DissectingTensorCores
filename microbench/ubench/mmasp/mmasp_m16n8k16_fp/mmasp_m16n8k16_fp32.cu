...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    ...
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32  microbenchmark with ILP = " << ILPconfig << std::endl;
    for(auto &e:warps){
