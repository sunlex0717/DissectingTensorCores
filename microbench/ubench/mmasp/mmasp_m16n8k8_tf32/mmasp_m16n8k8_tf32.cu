...
static_assert(ILPconfig<=4,"ILP> 4 is not spported");


__global__ void mmasp_1688(uint64_t *startClk, uint64_t *stopClk, float *a, float *b, uint32_t* meteE, float *res,
  ...
  a = a + warpid * 16*4; // m*k/2 = 16*4
  b = b + warpid * 8*8; // n*k = 8*8
  res = res + warpid * 16*8;// m*n = 16*8

   /** step 1: create register for each thread **/
   float frag_A[2*ILPconfig]; // two b32 registrs, 4 half non-zero elements, 16 dense 
   float frag_B[2*ILPconfig]; // two f16x2 registers, 8 half dense elements
...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    for (uint32_t i = 0; i < mma_k*mma_n; i++) {
      data2[i] = (T)i;
    }
    ...
    mmasp_1688<<<BLOCKS_NUM, THREADS_PER_BLOCK>>>(
    ...
    std::cout << "mma.sp.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sp.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32  microbenchmark with ILP = " << ILPconfig << std::endl;
