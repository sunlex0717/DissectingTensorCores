...
__global__ void mmasp_16816(uint64_t *startClk, uint64_t *stopClk, half *a, half *b, uint32_t* meteE, half *res,
  ...
  a = a + warpid * 16*8; // m*k/2 = 16*8
  b = b + warpid * 8*16; // n*k = 8*16
  res = res + warpid * 16*8;// m*n = 16*8

   /** step 1: create register for each thread **/
   half frag_A[4*ILPconfig]; // two b32 registrs, 4 half non-zero elements, 16 dense 
   half frag_B[4*ILPconfig]; // two f16x2 registers, 8 half dense elements
...
//   int intC = threadIdx.x + 2;
  ...
  for (int j = 0; j < ITERS; ++j) {
    ...
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[2]), "=r"(D[3]) //, "=f"(D[6]), "=f"(D[7])
      : "r"(A[2]), "r"(A[3]), 
        "r"(B[2]), "r"(B[3]),
        "r"(C[2]), "r"(C[3]), //"f"(C[6]), "f"(C[7]),
        "r"(E[1])
    );
    #endif
    #if (ILPconfig >= 3) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[4]), "=r"(D[5]) //, "=f"(D[10]), "=f"(D[11])
      : "r"(A[4]), "r"(A[5]), 
        "r"(B[4]), "r"(B[5]),
        "r"(C[4]), "r"(C[5]), //"f"(C[10]), "f"(C[11]),
        "r"(E[2])
    );
    #endif
    #if (ILPconfig >= 4) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[6]), "=r"(D[7]) //, "=f"(D[14]), "=f"(D[15])
      : "r"(A[6]), "r"(A[7]), 
        "r"(B[6]), "r"(B[7]),
        "r"(C[6]), "r"(C[7]), //"f"(C[14]), "f"(C[15]),
        "r"(E[3])
    );
    #endif
    #if (ILPconfig >= 5) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[8]), "=r"(D[9]) //, "=f"(D[18]), "=f"(D[19])
      : "r"(A[8]), "r"(A[9]), 
        "r"(B[8]), "r"(B[9]),
        "r"(C[8]), "r"(C[9]), //"f"(C[18]), "f"(C[19]),
        "r"(E[4])
    );
    #endif
    #if (ILPconfig >= 6) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[10]), "=r"(D[11]) //, "=f"(D[22]), "=f"(D[23])
      : "r"(A[10]), "r"(A[11]), 
        "r"(B[10]), "r"(B[11]),
        "r"(C[10]), "r"(C[11]), //"f"(C[22]), "f"(C[23]),
        "r"(E[5])
    );
    #endif
    #if (ILPconfig >= 7) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[12]), "=r"(D[13]) //, "=f"(D[26]), "=f"(D[27])
      : "r"(A[12]), "r"(A[13]), 
        "r"(B[12]), "r"(B[13]),
        "r"(C[12]), "r"(C[13]), // "f"(C[26]), "f"(C[27]),
        "r"(E[6])
    );
    #endif
    #if (ILPconfig >= 8) 
    asm volatile(
      "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16 {%0,%1}, "
      "{%2,%3}, {%4,%5}, {%6,%7}, %8, 0x0;\n"
      : "=r"(D[14]), "=r"(D[15]) //, "=f"(D[30]), "=f"(D[31])
      : "r"(A[14]), "r"(A[15]), 
        "r"(B[14]), "r"(B[15]),
        "r"(C[14]), "r"(C[15]), // "f"(C[30]), "f"(C[31]),
        "r"(E[7])
    );
    #endif
    __syncwarp();
...
float run(int THREADS_PER_BLOCK, bool report_fma_bw = false) {
    ...
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16  latency " << (float)total_time/(float)ITERS << " cycles\n";
...
int main() {
    ...
    intilizeDeviceProp(0);
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sp.sync.aligned.m16n8k16.row.col.f16.f16.f16.f16  microbenchmark with ILP = " << ILPconfig << std::endl;
    for(auto &e:warps){
