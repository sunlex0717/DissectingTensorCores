...
__global__ void shared_lat(uint32_t *startClk, uint32_t *stopClk,
  ...
  for (uint32_t i = uid; i < (SHARED_MEM_SIZE - stride); i += n_threads)
    ...
    for (uint32_t i = 0; i < ITERS; ++i) {
        ...
        asm volatile ("wmma.load.a.sync.aligned.row.m16n16k16.shared.f16 {%0, %1, %2, %3, %4, %5, %6, %7}, [%8];" 
                    : "=r"(frag[0]), "=r"(frag[1]),"=r"(frag[2]),"=r"(frag[3]), "=r"(frag[4]), "=r"(frag[5]),"=r"(frag[6]),"=r"(frag[7]) 
                    : "r"(frag[0])     ); // first 11
    }
    ...
    startClk[uid] = start;
    stopClk[uid] = stop;
    dsink[uid] = frag[0] +  frag[1] + frag[2] + frag[3] + frag[4] + frag[5] + frag[6] + frag[7];
}
void test_with_different_thread(int THREADS_NUM){
  ...
  std::cout << THREADS_NUM/32 <<" warps, wmma.load.a.sync.aligned.m16n16k16.row.f16 latency " << lat <<" ( " <<(unsigned)(lat) << " ) " << std::endl;
