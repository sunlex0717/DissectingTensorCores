...
std::vector<double> gemm_m16n8k8_bf16(){
    ...
    gemm_m16n8k4_kernel<<<BLOCKS_NUM, nwarps*warp_size>>>(dev_matA,dev_matB,dev_matC,dev_matD);
...
int main(){
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k4.row.col.f32.tf32.tf32.f32 numeric errors w.r.t fp32 on cpu"  << std::endl;
