...
std::vector<double> bf16_addtion_innerProduct(random_generator & random_gen){
    ...
    // std::cout<<"CPU A[0,0] = " << host_matA[0] << " A[0,1] = " << host_matA[1]  <<std::endl;
    // std::cout<<"CPU B[0,0] = " << host_matB[0] << " B[1,0] = " << host_matB[inst_n]  <<std::endl;

    // std::cout<<"CPU res = " << cpu_res_baseline[0] <<std::endl;
    // std::cout<<"GPU res = " << gpu_m16n8k8[0] <<std::endl;
...
std::vector<double> bf16_addtion_accumulation(random_generator & random_gen){
    ...
    // C is always f32 according to mma instruction
    ...
    // int is_inf = 0;
    // l2_is_inf = std::isinf(l2_relative_err);
    // if(std::isinf(l2_relative_err) == 1){
    //     is_inf
    // }
    // double l2_relative_err_halfK = 0.0;
    double abs_err = 0.0;
...
int main(){
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = bf16_addtion_innerProduct(random_gen);
        l2_relative_err += errors[0];
        abs_err += errors[1];
        // if(abs_err!=0.0 && Init_With_BF16 == 1){
        //     std::cout<<" abs error != 0.0, it is  " <<abs_err <<std::endl;
        // }
        // l2_relative_err_halfK += errors[1];
    }
    //std::cout<<"mma bf16 with accum fp32 | addition of Inner Product | l2 numeric errors w.r.t fp32 = "<< l2_relative_err/ROUNDS << std::endl;
    std::cout<<"mma bf16 with accum fp32 | addition of Inner Product | abs numeric errors w.r.t fp32 = "<< abs_err/ROUNDS << std::endl;
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = bf16_addtion_accumulation(random_gen2);
    ...
    //std::cout<<"mma bf16 with accum fp32 | addition of accumulation | l2 numeric errors w.r.t fp32 ="<< l2_relative_err/ROUNDS << std::endl;
    std::cout<<"mma bf16 with accum fp32 | addition of accumulation | abs numeric errors w.r.t fp32 = "<< abs_err/ROUNDS << std::endl;
}
