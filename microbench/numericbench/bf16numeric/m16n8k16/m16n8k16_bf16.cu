...
__global__ void gemm_m16n8k16_kernel(op_AB* MatA,op_AB* MatB,op_CD* MatC, op_CD* MatD ){
    ...
    // m16 n8 k16
    op_AB frag_A[8]; // 16 * 16  / 32 = 8 * bf16
    op_AB frag_B[4]; // 8 * 16  / 32
...
std::vector<double> gemm_m16n8k16_bf16(){
    int BLOCKS_NUM = 1;
    int nwarps = 1;
    int warp_size = 32;
...
int main(){
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma.sync.aligned.m16n8k16.row.col.f32.bf16.bf16.f32 numeric errors w.r.t fp32 on cpu"  << std::endl;
    // gemm_m16n8k16_bf16();
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = gemm_m16n8k16_bf16();
