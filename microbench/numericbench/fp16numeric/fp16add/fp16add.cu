...
std::vector<double> fp16_addtion_innerProduct( random_generator &random_gen){
...
std::vector<double> fp16_addtion_accumulation(random_generator &random_gen){
...
int main(){
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = fp16_addtion_innerProduct(random_gen);
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = fp16_addtion_accumulation(random_gen2);
        l2_relative_err += errors[0];
        abs_err += errors[1];
        abs_err_fp16accum += errors[2];
        abs_err_fp16accum_cvt_half += errors[3];
