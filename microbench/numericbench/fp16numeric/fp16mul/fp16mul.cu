...
__global__ void gemm_m16n8k8_fp32accum_kernel(float* MatA,float* MatB,float* MatC, float* MatD ){
    ...
    // m16 n8 k16
    half frag_A[4]; // 16 * 16  / 32 = 8 * bf16
    half frag_B[2]; // 8 * 16  / 32
...
void gpu_fp16_fp32accum_m16n8k8(float* matA_in, float * matB_in, float* matC_in, float* matD_out){
...
void gpu_fp16_fp16accum_m16n8k8(float* matA_in, float * matB_in, float* matC_in, float* matD_out){
...
std::vector<double> fp16mul(random_generator & random_gen){
    ...
    float *gpu_m16n8k8_fp32accum = (float *)malloc(total_C_SIZE * sizeof(float));
    float *gpu_m16n8k8_fp16accum = (float *)malloc(total_C_SIZE * sizeof(float));
    ...
    gpu_fp16_fp32accum_m16n8k8(host_matA,host_matB,host_matC,gpu_m16n8k8_fp32accum);

    gpu_fp16_fp16accum_m16n8k8(host_matA,host_matB,host_matC,gpu_m16n8k8_fp16accum);
    ...
    double abs_err = 0.0;
    compute_diff_l2_norm(&cpu_res_baseline[0],&gpu_m16n8k8_fp32accum[0],1,1,l2_relative_err);



    double abs_err_fp16accum = 0.0;
    double abs_err_fp16accum_cvt_half = 0.0;
    abs_err = std::abs(double(cpu_res_baseline[0]) - double(gpu_m16n8k8_fp32accum[0]));
    abs_err_fp16accum = std::abs(double( (cpu_res_baseline[0]) ) - double(gpu_m16n8k8_fp16accum[0]));
    abs_err_fp16accum_cvt_half = std::abs(double( half(cpu_res_baseline[0]) ) - double(gpu_m16n8k8_fp16accum[0]));
...
int main(){
    ...
    for(int i=0;i < ROUNDS; i ++){
        std::vector<double> errors = fp16mul(random_gen);
    ...
    std::cout<<"mma fp16 with accum fp32 | multiplication | abs numeric errors w.r.t fp32 on CPU = "<< abs_err/ROUNDS << std::endl;
    std::cout<<"mma fp16 with accum fp16 | multiplication | abs numeric errors w.r.t fp32 on CPU = "<< abs_err_fp16accum/ROUNDS << std::endl;
    std::cout<<"mma fp16 with accum fp16 | multiplication | abs numeric errors w.r.t fp32 on CPU coverted to half = "<< abs_err_fp16accum_cvt_half/ROUNDS << std::endl;
