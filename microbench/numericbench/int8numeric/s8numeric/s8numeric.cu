...
__global__ void gemm_m16n8k16_s8_kernel(int8_t* MatA,int8_t* MatB,float* MatC, float* MatD ){
    ...
    // m16 n8 k16
    int8_t frag_A[8]; // 16 * 16  / 32 = 8 * half
    ...
    for(int i =0; i < 4; i++){
        ...
        // row major
        frag_A[i] = (half)MatA[inst_k*row_a + col_a];
    ...
    for(int i =0; i < 2; i++){
        ...
        // row-major B
        frag_B[i] = (half)MatB[row_b*inst_n + col_b];
    }
...
std::vector<double> bf16_chain_matmul(int n, random_generator & random_gen){
    ...
    for(int chain_id =0; chain_id < num_chain; chain_id ++){
        ...
        if(Mat_has_inf(cpu_res_baseline,inst_m*inst_n)){
            ...
            std::cout<<"CPU baseline matrix contains inf, break at chain = "<< chain_id<<std::endl;
            exit(0);
        }
        if(Mat_has_inf(gpu_m16n8k8,inst_m*inst_n)){
            
            std::cout<<"GPU matrix contains inf, break at chain = "<< chain_id<<std::endl;
            exit(0);
        }
    ...
    double l1_relative_err = 0.0;
    double abs_sum_err = 0.0;
    compute_diff_l2_norm(cpu_res_baseline,gpu_m16n8k8,inst_m,inst_n,l2_relative_err);

    //(float* cpu_base, float* gpu_res, int rows, int cols,double& abs_err, double&  l1_norm)
    compute_diff_l1_norm(cpu_res_baseline,gpu_m16n8k8,inst_m,inst_n,abs_sum_err,l1_relative_err);
    ...
    std::vector<double> res{l2_relative_err,abs_sum_err,l1_relative_err};
    return res;
}
...
int main(){
    ...
    // const int inst_k = 8;
    std::cout<<"***********************************"<<std::endl;
    std::cout << "mma fp16 numeric errors with chain matmul"  << std::endl;
    if(Init_With_FP16 == 1){
    ...
    for(int num_chain =1; num_chain<=CHAINs; num_chain++){
        ...
        for(int i=0;i < ROUNDS; i ++){
            ...
            abs_sum_err += errors[1];
            l1_relative_err += errors[2];
        }
        std::cout<<"length of chain = "<< num_chain <<", mma.m16n8k8.fp16 l2 relative error :"<< l2_relative_err/ROUNDS << std::endl;
        // std::cout<<"length ofchain = "<< num_chain <<", mma.m16n8k8.fp16 abs_sum_err relative error :"<< abs_sum_err/ROUNDS << std::endl;
        // std::cout<<"length ofchain = "<< num_chain <<", mma.m16n8k8.fp16 l1_relative_err relative error :"<< l1_relative_err/ROUNDS << std::endl;
        std::cout<<"***********************************"<<std::endl;
