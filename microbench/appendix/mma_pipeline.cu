// todo implement pipelined version 
...
const static unsigned int NUM_PIPE_STAGES = 2;
...
__global__ void 
mma_pipeline(const __nv_bfloat16* __restrict__   matA, const __nv_bfloat16* __restrict__  matB, 
            float* matD,long long int* d_gpu_clock /*, __nv_bfloat16* fragementA_global_vis*/)
{
    ...
    auto this_thread = cooperative_groups::this_thread();
    ...
    extern __shared__ __nv_bfloat16 buffer [][BLOCK_Tile_M+BLOCK_Tile_N][BLOCK_Tile_K + SKEW_BF16];
    ...
    for(unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x){
        ...
        for(int compute_stage =0, fetch_stage =0; compute_stage < (K_GLOBAL/BLOCK_Tile_K);compute_stage++){

            for(;fetch_stage<(K_GLOBAL/BLOCK_Tile_K) && fetch_stage <(compute_stage+NUM_PIPE_STAGES);fetch_stage++){
                pipeline.producer_acquire();
                // copy here
                size_t fetch_pipe_idx = fetch_stage % NUM_PIPE_STAGES;
                ...
                for(int i=0;i<num_iters_ATile;i++){
                    cuda::memcpy_async(this_thread,((int4*)&buffer[fetch_pipe_idx][shmem_idx_lane + i*Block_TileK_Copy_Lines_Per_Warp ][0] + (laneId%Block_TileK_Copy_Line_LANEs)),((int4*)&matA[(global_idx_lane_m + i*Block_TileK_Copy_Lines_Per_Warp)*K_GLOBAL + fetch_stage*BLOCK_Tile_K] +  global_idx_lane_k),sizeof(int4),pipeline);
                }
                ...
                for(int j=0;j<num_iters_BTile;j++){
                    cuda::memcpy_async(this_thread,((int4*)&buffer[fetch_pipe_idx][shmem_idx_lane + j*Block_TileK_Copy_Lines_Per_Warp + BLOCK_Tile_M ][0] + (laneId%Block_TileK_Copy_Line_LANEs)),((int4*)&matB[(global_idx_lane_n + j*Block_TileK_Copy_Lines_Per_Warp)*K_GLOBAL + fetch_stage*BLOCK_Tile_K] +  global_idx_lane_k),sizeof(int4),pipeline);
                }
            ...
            this_block.sync();
            size_t compute_pipe_idx = compute_stage % NUM_PIPE_STAGES;
            ...
            for(int k_mma_step =0; k_mma_step<(BLOCK_Tile_K/MMA_Tile_K);k_mma_step++){
                ...
                for(int mma_m =0; mma_m<WARP_Tile_M/MMA_Tile_M;mma_m++){
                    ...
                    unsigned  tile_matA_shared_ptr= static_cast<unsigned>(__cvta_generic_to_shared(&buffer[compute_pipe_idx][warpId_m*WARP_Tile_M + mma_m *MMA_Tile_M+ laneId%16 ][(laneId/16) * 8 + k_mma_step*MMA_Tile_K]));
                    ...
                    for(int mma_n = 0; mma_n<WARP_Tile_N/MMA_Tile_N;mma_n++){
                        ...
                        unsigned  tile_matB_shared_ptr= static_cast<unsigned>(__cvta_generic_to_shared(&buffer[compute_pipe_idx][warpId_n*WARP_Tile_N + mma_n*MMA_Tile_N+ laneId%8 + BLOCK_Tile_M ][(laneId/8) * 8 + k_mma_step*MMA_Tile_K]));
        ...
        //     printf("num cycles of mma pipeline %lld\n", end_t-start_t);
        // }
...
int main(){
    ...
    int size_shmem_per_block_bytes = NUM_PIPE_STAGES * (BLOCK_Tile_M*(BLOCK_Tile_K+SKEW_BF16) + BLOCK_Tile_N*(BLOCK_Tile_K+SKEW_BF16)) * sizeof(__nv_bfloat16);
    ...
    for(int iter=0; iter<NUM_PROFILES; ++iter){
            ...
            mma_pipeline<<<num_blocks,num_threads_per_block,size_shmem_per_block_bytes>>>(d_matA,d_matB,d_matD,d_gpu_clock /*,d_fragementA_vis*/); 
    ...
    printf("num of cycles mma pipeline: %lld\n", baseline_cycles);  

    //mma_pipeline<<<num_blocks,num_threads_per_block,size_shmem_per_block_bytes>>>(d_matA,d_matB,d_matD /*,d_fragementA_vis*/); 
