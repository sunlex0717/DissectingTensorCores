...
__global__ void 
mma_baseline(const __nv_bfloat16* __restrict__   matA, const __nv_bfloat16* __restrict__  matB, 
    ...
    for(unsigned int block_pos = blockIdx.x;; block_pos += gridDim.x){
        ...
        for(int blk_tile_k =0; blk_tile_k < (K_GLOBAL/BLOCK_Tile_K);blk_tile_k++){
            ...
            for(int i=0;i<num_iters_ATile;i++){
                *((int4*)&buffer[shmem_idx_lane + i*Block_TileK_Copy_Lines_Per_Warp ][0] + (laneId%Block_TileK_Copy_Line_LANEs)) =  *((int4*)&matA[(global_idx_lane_m + i*Block_TileK_Copy_Lines_Per_Warp)*K_GLOBAL + blk_tile_k*BLOCK_Tile_K] +  global_idx_lane_k);
            }
            ...
            for(int j=0;j<num_iters_BTile;j++){
                *((int4*)&buffer[shmem_idx_lane + j*Block_TileK_Copy_Lines_Per_Warp + BLOCK_Tile_M ][0] + (laneId%Block_TileK_Copy_Line_LANEs)) =  *((int4*)&matB[(global_idx_lane_n + j*Block_TileK_Copy_Lines_Per_Warp)*K_GLOBAL + blk_tile_k*BLOCK_Tile_K] +  global_idx_lane_k);
            }
            /******************************* shared mem store************************************/
            __syncthreads();
...
int main(){
    ...
    for(int iter=0; iter<NUM_PROFILES; ++iter){
            ...
            mma_baseline<<<num_blocks,num_threads_per_block,size_shmem_per_block_bytes>>>(d_matA,d_matB,d_matD,d_gpu_clock /*,d_fragementA_vis*/); 
